...
# Enables coverage data collection for the microservices
...
if (-not (Test-Path $JAVA_EXE)) {
    Write-Host "ERROR: Java not found at: $JAVA_EXE"
    exit 1
}
...
Write-Host "OK: JaCoCo agent found at: $JACOCO_AGENT"

# Step 1: Stop existing services
if (-not $SkipKill) {
    Write-Host ""
    Write-Host "Step 1: Stopping existing Java services..."
    taskkill /IM java.exe /F 2>$null | Out-Null
    Start-Sleep -Seconds 3
    Write-Host "Done: Services stopped"
}

# Step 2: Start Conge Service with JaCoCo
Write-Host ""
Write-Host "Step 2: Starting Leave Service (conge) on port 9000..."
...
$CONGE_JACOCO = "-javaagent:$JACOCO_AGENT=destfile=$OUTPUT_DIR\conge.exec,append=false"
Write-Host "Starting with JaCoCo agent..."
...
Write-Host "OK: Leave Service started (PID: $($CONGE_PROCESS.Id))"
Start-Sleep -Seconds 5

# Step 3: Start Auth Service with JaCoCo
Write-Host ""
Write-Host "Step 3: Starting Auth Service (DemandeConge) on port 9001..."
...
$AUTH_JACOCO = "-javaagent:$JACOCO_AGENT=destfile=$OUTPUT_DIR\auth.exec,append=false"
Write-Host "Starting with JaCoCo agent..."
...
Write-Host "OK: Auth Service started (PID: $($AUTH_PROCESS.Id))"
Start-Sleep -Seconds 5
...
Write-Host "  Port: 9000, PID: $($CONGE_PROCESS.Id)"
Write-Host "  JaCoCo: $OUTPUT_DIR\conge.exec"
Write-Host ""
Write-Host "Auth Service (DemandeConge):"
Write-Host "  Port: 9001, PID: $($AUTH_PROCESS.Id)"
Write-Host "  JaCoCo: $OUTPUT_DIR\auth.exec"
Write-Host ""
Write-Host "NEXT: Run the test pipeline to collect coverage"
